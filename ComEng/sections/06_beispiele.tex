\section{Beispiele}\vspace{-0.3em}
...
%     \vspace{0pt} % Erzwingt Ausrichtung oben
%     \centering
%     \includegraphics[width=\linewidth]{images/triple_if_else.png}
% \end{minipage}% <-- Das % verhindert zusätzliche horizontale Leerzeichen
% \hfill
% \begin{minipage}[t]{0.72\columnwidth}
%     \vspace{0pt}
%     \setlength{\fboxsep}{2pt} % Reduziert die Höhe der violetten Box
...
\vspace{1ex} % Reduzierter Abstand zwischen den beiden Beispielen
...
\begin{enumerate}
    \item \textbf{Kontrolle der Grenzen (Bound Check):} Anfangsprüfung des Werts von \texttt{State}, um sofort zum \texttt{default} zu springen, falls der Index außerhalb des zulässigen Bereichs liegt.
    \item \textbf{Berechnung des Offsets:} Multiplikation des Werts mit 4 ($State \ll 2$) mittels logischem Shift, da jeder Zeiger in der Look-up-Tabelle 32 Bit (4 Byte) belegt.
    \item \textbf{Abruf des Zeigers:} Lesen der exakten Zieladresse, indem der berechnete Offset zur Basisadresse der Tabelle addiert wird.
    \item \textbf{Sprung (Branch):} Laden der abgerufenen Adresse in den Program Counter (\texttt{PC}), um die Ausführung des spezifischen Falls zu starten.
\end{enumerate}
