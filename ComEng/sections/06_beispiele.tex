...
% \end{minipage}% <-- Il % impedisce spazi orizzontali aggiuntivi
% \hfill
% \begin{minipage}[t]{0.72\columnwidth}
%     \vspace{0pt}
%     \setlength{\fboxsep}{2pt} % Riduce l'altezza della casella viola
...
\vspace{1ex} % Distanza ridotta tra i due esempi
...
\begin{enumerate}
    \item \textbf{Controllo dei limiti (Bound Check):} Controllo iniziale del valore di \texttt{State} per saltare immediatamente a \texttt{default} se l'indice si trova al di fuori dell'intervallo consentito.
    \item \textbf{Calcolo dell'offset:} Moltiplicazione del valore per 4 ($State \ll 2$) mediante shift logico, poiché ogni puntatore nella look-up table occupa 32 bit (4 byte).
    \item \textbf{Recupero del puntatore:} Lettura dell'indirizzo di destinazione esatto, aggiungendo l'offset calcolato all'indirizzo di base della tabella.
