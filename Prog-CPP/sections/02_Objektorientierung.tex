...
\begin{enumerate}[itemsep=1pt, parsep=0pt]
    ...
    \item \textit{\color{blue} \texttt{Includes} des verwendete System Header}
    \item \textit{\color{blue} \texttt{Includes} der Projektbezogenen Header}
...
Die \texttt{Unified Modeling Language} ist eine normierte Sprache um Objekte grafisch darzustellen. 
...
\begin{center}
    \begin{tikzpicture}[
        ...
        \node [umlclass] (Student) {
            \centering \textbf{Student} % Centriamo solo il nome
            \nodepart{second}
                + IDNumber : int \\
                \# name : std::string \\
                - bierkonsum : float \\
                \# investedTimeInET : long int
            \nodepart{third}
                - doStuff(time : int\&) : void \\
                + \textit{useless(void)} : retType*
        };
    \end{tikzpicture}
    % \includegraphics[width=0.8\columnwidth]{images/UML-Klassendiagramm Notation.png}    
\end{center}
\end{minipage}
...
\begin{itemize}[itemsep=1pt, parsep=0pt]
    \item \textbf{\texttt{Keine}} : Default Konstruktor
    \item \textbf{\texttt{const-Referenz auf eigene Klasse}} : copy Konstruktor
...
\begin{itemize}[itemsep=1pt, parsep=0pt]
    ...
    \item sonstige $\rightarrow$ werden anhand ihrer Aufrufparameter unterschieden, Sprechweise:\texttt{überladen}
\end{itemize}
...
\begin{center}
    \includegraphics[width=0.7\columnwidth]{images/Vererbung.png}    
\end{center}
...
\begin{center}
        \includegraphics[width=\columnwidth]{images/vtable.png}
\end{center}
