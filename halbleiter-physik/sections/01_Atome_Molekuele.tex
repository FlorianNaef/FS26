...
\subsubsection{Rydberg-Formel ($\Delta E$ für Elektronensprünge)}
Diese Formel berechnet die Energiedifferenz, die beim "Sprung" eines Elektrons zwischen zwei Niveaus in einem wasserstoffähnlichen Atom entsteht.
...
\begin{minipage}{0.63\columnwidth}
	\begin{itemize}
    \item $Z$: Ordnungszahl des Elements.
    \item $E_I$: Ionisierungsenergie (Wasserstoff $= 13.6 \text{ eV}$).
    \item $n_j$: Quantenzahl des Startniveaus.
    \item $n_i$: Quantenzahl des Zielniveaus.
\end{itemize}
\end{minipage}
...
\textbf{Per Konvention füllt man Kästchen zuerst mit Spin nach oben $\uparrow$\vspace{1em}}
...
\begin{tabular}{|l|l|c|}
\hline
\textbf{Kategorie} & \textbf{Struktur} & \textbf{Atome pro Zelle ($n$)} \\ \hline
...
$ \underbrace{\sqrt{3a^2}}_{\text{Würfeldiagonale}} = \underbrace{4 \cdot R}_{\text{Atomradius}} 
\quad \Leftrightarrow a = \frac{4R}{\sqrt{3}} \qquad \Rightarrow 
\quad (a) = \text{Gitterkonstante} $


% --- Densità ---
\subsubsection{Materialdichte ($\rho$)}

Die Dichte ist das Verhältnis von Gesamtmasse der Atome in der Zelle zum Zellvolumen:
...
\begin{minipage}{0.6\columnwidth}
	\begin{itemize}
		\item $n$: Anzahl Atome pro Elementarzelle.
		\item $M$: Atommasse $[\frac{\kilogram}{\mole}]$.
		\item $a$: Gitterkonstante
		\item $N_A \approx 6.022 \times 10^{23} \, \text{mol}^{-1}$ (Avogadro-Konstante).
	\end{itemize}
\end{minipage}

% --- Concentrazione Atomica ---
\subsubsection{Atomkonzentration ($n_{at}$)}
...
Volumenanteil, der von den Atomen belegt wird (starre Kugeln):

$$P = \frac{n \cdot V_{atomo}}{V_{cella}} = \frac{n \cdot \frac{4}{3}\pi R^3}{a^3}$$


% --- Densità Gas di Elettroni ---
\subsubsection{Dichte des freien Elektronengases ($n_e$)}
Anzahl der Leitungselektronen pro Volumeneinheit:

$$n_e = Z \cdot n_{at}$$

$Z$ ist die Anzahl der Elektronen, die jedes Atom an das "Gas" abgibt.
