...
\subsection{Photoelektrischer Effekt}
	...
	\begin{minipage}{0.4\linewidth}
		\begin{itemize}
			\item I fotoni che colpiscono la piastra fanno uscire elettroni.
			\item La corrente viene determinata \textbf{solo} dall'intensità della luce (N. fotoni)
			\item $V_0$ (l'$E_{kin}$ degli elettroni) dipende \textbf{solo} dalla lunghezza d'onda		\end{itemize}
	\end{minipage}
...
\begin{tabular}{lll}
    ...
    Potenza interna & $\displaystyle P_{int} = \frac{P_{out}}{T}$ & $T = \text{Trasmissione (in \%)}$ \\ 
    Energia del risonatore & $\displaystyle E_k = 2 \cdot P_{int} \cdot \frac{l}{c}$ & 2 perché va avanti e indietro\\\end{tabular}
