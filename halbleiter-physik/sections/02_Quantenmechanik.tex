...
\subsection{Photoelektrischer Effekt}
	...
	\begin{minipage}{0.4\linewidth}
		\begin{itemize}
			\item Die auf die Platte treffenden Photonen schlagen Elektronen heraus.
			\item Der Strom wird \textbf{nur} durch die Lichtintensität bestimmt (Anz. Photonen)
			\item $V_0$ (die $E_{kin}$ der Elektronen) hängt \textbf{nur} von der Wellenlänge ab
		\end{itemize}
	\end{minipage}
...
\begin{tabular}{lll}
    ...
    Interne Leistung & $\displaystyle P_{int} = \frac{P_{out}}{T}$ & $T = \text{Transmission (in \%)}$ \\ 
    Resonatorenergie & $\displaystyle E_k = 2 \cdot P_{int} \cdot \frac{l}{c}$ & 2 da es hin und her wandert\\
\end{tabular}
