...
Multiplikation -> einer Drehung um den Winkel $\arg(b)$ und einer Streckung um $\vert b \vert$.
...
\subsubsection{Ziehen der $n$-ten Wurzel (Radizieren)}

\begin{itemize}
    \item \textbf{Eigenschaft:} Die Distributivgesetze der Wurzeln \textbf{gelten generell nicht} im komplexen Bereich ($\sqrt{ab} \neq \sqrt{a}\sqrt{b}$ in $\mathbb{C}$).
    \item \textbf{Vielfachheit:} Jede komplexe Zahl $a \neq 0$ besitzt exakt $n$ verschiedene $n$-te Wurzeln in $\mathbb{C}$.
    \item \textbf{Geometrie:} Die Lösungen bilden in der Ebene die Ecken eines regulären \textbf{$n$-Ecks}, zentriert im Ursprung auf einem Kreis mit Radius $R = \sqrt[n]{|a|}$.
...
\example{Bestimmung der vierten Wurzeln von $-16$ ($z^4 = -16$)}

\begin{minipage}{0.58\columnwidth}
    \begin{enumerate}
        \item \textbf{Betrag ($|z_k|$):}
              \begin{align*}
                  \lvert z \rvert^4 \cdot \cjs(4 \varphi) & = 16 \cdot \cjs(\pi) \\
                  z = \sqrt[4]{\lvert 16 \rvert}          & = 2
              \end{align*}
        \item \textbf{Hauptargument ($z_0$):}

              Man teilt das Argument $\arg(a) = \pi$ durch den Index $n=4$:
              \begin{equation*}
                  \alpha_0 = \frac{\arg(a)}{n} = \frac{\pi}{4} = 45^\circ
              \end{equation*}
        \item \textbf{Folgeargumente:}
              $a_n = a_{n-1} + \Delta\varphi$ \\
              $\Delta\varphi = \frac{2\pi}{n}$ oder $\frac{360^{\circ}}{n} \,\, \frac{2\pi}{4} = \frac{\pi}{2}$:
              \begin{align*}
...
\begin{minipage}{0.38\columnwidth}
    ...
    \begin{align*}
        z   & = r \cdot \e^{\varphi \jimg} = r \cdot \cjs(\varphi) \\
...
% Optimierte Version des Steiner-Formalismus
Mit $a = r \mathrm{e}^{\jimg \varphi}$ ist die Menge der $n$-ten Wurzeln definiert als:
\begin{equation*}
...
Die Exponentialgesetze bleiben in $\mathbb{C}$ für \myul{ganzzahlige} Exponenten erhalten! \smallskip
...
\[\sinh(\alpha \cdot z) = \frac{\e^{\alpha z} - \e^{-\alpha z}}{2} \qquad
    \cosh(\alpha \cdot z) = \frac{\e^{\alpha z} + \e^{-\alpha z}}{2} \qquad
...
\begin{enumerate}[itemsep=0.2cm]
    \item Komplexe Lösungen können völlig asymmetrisch sein und nicht als Paar auftreten
    ...
    \item Ein komplexes Polynom vom Grad $n$ zerfällt in $\mathbb{C}$ in lauter Linearfaktoren. \\
          Die Faktoren müssen \textbf{nicht} verschieden sein. Die Zerlegung ist bis auf die Reihenfolge eindeutig. \\
...
\para{Bestimmung des Vorfaktors $a_n$}

Um den Hauptkoeffizienten \textbf{$a_n$} zu bestimmen:
\begin{enumerate}
    \item \textbf{Aus der Gleichung:} Es ist der Koeffizient des Terms mit der höchsten Potenz ($z^n$).
    \item \textbf{Durch Einsetzen:} Kennt man die Wurzeln $z_i$ und einen Punkt $P(z_0) = y_0$, dann:
...
\textbf{Anzahl Nullstellen:} \\
Polynom Grad n >= 1 hat n Nullstellen\\
Reelle Koeffizienten: n = r + 2k (Reelle Nullstellen + Komplexe Nullstellen Paare)\\
Komplexe Koeffizienten: Paarbildung entfällt, r ist maximal n-1 wenn das Polynom im normierten Fall (Leitkoeffizient = 1) immer noch komplexe Koeffizienten hat.
...
\begin{tabular}{lll@{}}
    ...
    'Schwingung' / 'Rotation'
                                & $z(t) = A \cdot \e^{\jimg (\omega t + \varphi)} = \underbrace{A \cdot \e^{\jimg \varphi}}_{z_i(0)} \cdot \e^{\jimg \omega t}$ & (zeitabhängig)
\end{tabular}
...
\begin{itemize}
    ...
    \item Die Summe nur mit $\sin \text{oder} \cos$ berechnen, eines wählen und das andere mit Trigo-Eigenschaften umformen (\ref{chap:trigo-beziehungen})
    \item Um $A$ zu finden: exp $\rightarrow$ kartesisch, $\sum$ berechnen, Resultat von kartesisch $\rightarrow$ exp
\end{itemize}
