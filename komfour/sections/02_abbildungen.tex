...
\begin{itemize}[itemsep=0.2cm]
    \item Winkeltreue für komplex differenzierbare Funktion mit $f'(z) \neq 0$ \\
          ...
          \textrightarrow\ Winkeltreue bedeutet, dass der Schnittwinkel zwischen Geraden erhalten bleibt
...
Parametergleichung einer Bildgeraden kann \textbf{immer} ermittelt werden! \smallskip
...
\begin{itemize}
    \item $c_1$ ist der Stützvektor auf der reellen Achse, $j$ die vertikale Richtung.
    \item $j c_2$ ist der Stützvektor auf der imaginären Achse, $1$ die horizontale Richtung.
\end{itemize}
...
\begin{enumerate}
    \item Real- und Imaginärteil trennen ($w_1 = \RE(w), w_2 = \IM(w)$)
    \item Gleichungssystem lösen und Parameter $r, s$ entfernen
\end{enumerate}
...
\begin{itemize}[itemsep=0.1cm]
    ...
    \item Man kann $a$ auch als Streckung und Drehung um $O$ sehen, $b$ als Verschiebung
\end{itemize}
...
\begin{minipage}[t]{0.43\linewidth}
    ...
    \textbf{Orthogonalität und Testpunkt}
    \begin{itemize}
        \item \textbf{Fixkreise:} Ein Kreis, der den Einheitskreis orthogonal ($90^\circ$) schneidet, wird auf sich selbst abgebildet.
    \end{itemize}
\end{minipage}

\begin{itemize}
    \item \textbf{Testpunkt:} Um die zu färbende Fläche zu bestimmen, einen Testpunkt verwenden.
    \item \textbf{Vorzeichen von Im:} Bei $1/z$ (Kehrwert) wechselt das Vorzeichen des Imaginärteils; bei $1/\bar{z}$ (Inversion) bleibt es gleich.
\end{itemize}
...
$$ f: \: z \mapsto w = \frac{1}{z} = \overline{\left(\frac{1}{\overline{z}}\right)} $$
...
$\text{Tipp: bei j im Nenner mit j erweitern:} \frac{-3}{5j} = \frac{-3j}{5j^2} = \frac{3}{5}j$
