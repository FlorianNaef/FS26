...
\scalebox{0.33}{
    ...
    \begin{tabularx}{3\columnwidth}{rp{1mm}V{3}*{17}{C}}
        ...
        $\bm{\tan(\alpha)}$                                              &             & $0$                           & $\dfrac{\sqrt{3}}{3\mathstrut}$ & $1$                   & $\sqrt{3}$                      & $\pm\infty$
                                                                         & $-\sqrt{3}$ & $-1$                          & $-\dfrac{\sqrt{3}}{3}$          & $0$                   & $\dfrac{\sqrt{3}}{3}$           & $1$              & $\sqrt{3}$             & $\pm\infty$            & $-\sqrt{3}$            & $-1$        & $-\dfrac{\sqrt{3}}{3}$ & $0$                                                                                                                                                         \\\hline
...
\[\sin(\alpha) = \frac{\rm Gegenkathete}{\rm Hypotenuse} \qquad \qquad \cos(\alpha) = \frac{\rm Ankathete}{\rm Hypotenuse}\]
...
\begin{tabular}{l}
    ...
    $\sin(a) \cdot \cos(b) = \frac{1}{2} \big( \sin(a-b) + \sin(a+b) \big) = \frac{1}{2} \big( \sin(a+b) - \sin(b-a) \big) $
\end{tabular}
\renewcommand{\arraystretch}{1}
...
\subsubsection{Wichtige Werte für Fourierreihen}

% --- Grundwerte für n als Ganzzahl ---
\noindent\textbf{Grundwerte ($n \in \mathbb{Z}$):}
\begin{center}
    \begin{tabular}{lll}
...
% --- Fälle mit Pi-Brüchen (2 Spalten kompakt) ---
\noindent\textbf{Bruchteile von $\pi$ ($n \in \mathbb{Z}$, mit $k \in \mathbb{Z}$):}
\begin{center}
    \begin{tabular}{ll}
...
% \vspace{0.2cm}
% % --- Linearkombinationen (verschobene Koeffizienten) ---
% \noindent\textbf{Konstante Kombinationen in den Koeffizienten ($n \in \mathbb{Z}$):}
% \begin{center}
% \begin{tabular}{ll}
...
\begin{tabular}{lll}
    1. & Streckung um $\bm{\frac{1}{a}}$ in $x$-Richtung
       & $y = f(a \cdot x)$                                                               \\
...
\textbf{LIATE} - Priorität für $g$ - abzuleitende Funktion (nicht $\dots'$ in der Basisform):
...
\begin{center}
    ...
    \begin{tabular}{ccc}
        ...
        \midrule
        \cbl{$\e^{-\jimg \, n \, \omega_f \, t}$} & \cbl{$ \frac{\e^{- \jimg \, n \, \omega_f \, t}}{- \jimg \, n \, \omega_f} = \frac{1}{- \jimg \, n \, \omega_f} \e^{-\jimg \, n \, \omega_f \, t}$} & \cbl{$(n \neq 0)$} \\
        \midrule
        $t \, \sin(n \omega_f t)$                 & $\frac{\sin(n \omega_f t)}{(n \omega_f)^2} - \frac{t \cos(n \omega_f t)}{n \omega_f}$
                                                  & $(n \neq 0)$                                                                                                                                                             \\
        \midrule
        $t \cos{(n \omega_f t)}$                  & $\frac{\cos{(n \omega_f t)}}{(n \omega_f)^2} + \frac{t \sin(n \omega_f t)}{n \omega_f}$
                                                  & $(n \neq 0)$                                                                                                                                                             \\
        \bottomrule
    \end{tabular}
    \renewcommand{\arraystretch}{1}
...
\subsection{\textbf{Beispiel:} Überlagerung}
\textbf{Gegeben:} $x_1(t) = \cos(3t - \pi/6)$ und $x_2(t) = \sqrt{3}\cos(3t + 2\pi/3)$ \\
\textbf{Gesucht:} $x(t) = x_1(t) + x_2(t)$

\textbf{Lösung:}
\begin{itemize}
    \item[] $A_1 = \e^{-\jimg \pi/6} = \frac{\sqrt{3}}{2} - \jimg \frac{1}{2}$
    \item[] $A_2 = \sqrt{3}\cdot\e^{\jimg 2\pi/3} = \sqrt{3}\left(-\frac{1}{2} + \jimg \frac{\sqrt{3}}{2}\right) = -\frac{\sqrt{3}}{2} + \jimg \frac{3}{2}$
    \item[] $A = A_1 + A_2 = \jimg = \e^{\jimg \pi/2}$
    \item[] $x(t) = \RE\{ A \cdot \e^{\jimg 3t} \} = \cos(3t + \pi/2)$
\end{itemize}