...
\subsubsection*{Festkörper \normalfont (Feststoffe)}

\begin{itemize}
    \item Kein Fluid
    \item Festes Volumen; feste Gestalt (Form)
...
\subsubsection*{Ideale Flüssigkeit \normalfont (Flüssigkeiten)}

\begin{itemize}
    \item Fluid
    \item Festes Volumen; keine feste Gestalt (Form)
...
\begin{tabular}{c l c}
    ...
    $K = \frac{1}{\kappa}$  & Kompressionsmodul              
    & $[K] = \pascal$                                       \\
...
\begin{tabular}{c l c}
    $p(h)$      & Schweredruck des Gases bei Höhe $h$                    
                    & $[p(h)] = \pascal$                        \\
...
\textbf{Diese Formel ist auch anwendbar für einen Ursprung ungleich $0 \meter$.}

Verwende jedoch Drücke für spez. $\cor{h_0}$ und $\cor{h_{\rm rel}}$
...
Beim Schwimmen gilt $m_{\rm verdrängt} = m_{\rm Körper}$, für alle Fluide $V_{\rm verdrängt} \cdot \rho_{\rm Fl} = m_{\rm Körper}$  
...
\textbf{Die Oberflächenspannung $\sigma$ wirkt entlang der \textbf{Kontaktlinie} zwischen Flüssigkeit und Feststoff.}

\smallskip

\begin{tabular}{ll c|c ll}
    Zylinder & $l = 2 \, \pi \, r = \pi d$ (Kreisumfang)& &
