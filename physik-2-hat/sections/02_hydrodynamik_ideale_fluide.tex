...
Ist ein Ende offen, gilt $p=0$ (z.B. bei einer Spritze)

\subsubsection*{Abgeleitete Formeln}
...
Man verwendet das Symbol $\vec{I}$ für den Impuls, um Verwechslung mit Druck $p$ zu meiden.
\subsubsection{Grundprinzipien}
\begin{itemize}
    \item \textbf{Festkörper:} $\vec{I}$=$m \cdot \vec{v}$ Die zeitliche Ableitung des Impulses entspricht der Kraft: $\frac{d\vec{I}}{dt} = \vec{F}$
    \item \textbf{Fluide (Kontrollvolumen):} Der Gesamtimpuls in einem Volumen $V$ ist definiert als:
    $$\vec{I} = \iiint_V \rho \cdot \vec{v} \, dV$$
    \item \textbf{Erhaltungsgleichung:} Die Summe der äusseren Kräfte entspricht der Änderung des Impulsflusses:
    $$\sum \vec{F} = \frac{d\vec{I}}{dt}$$
\end{itemize}

\subsubsection{Impulsbilanz (Stationär)}
Für strömende Fluide im Rohr mit Volumenstrom $Q$ (oder $\dot{V}$) gilt die Bilanz Ein- (1) und Ausgang (2):
$$\rho Q \vec{v}_1 + \sum \vec{F} = \rho Q \vec{v}_2 \qquad \rightarrow \qquad \sum \vec{F} = \rho \dot{V} (\vec{v}_2 - \vec{v}_1)$$

\example{Aufteilung in $x$- und $y$-Komponenten}
\begin{minipage}{0.55\columnwidth}	
	\begin{itemize}
		\item Grundgleichung:
		$$\underbrace{\rho Q \vec{v}_1}_{\vec{I}\text{ im Eingang}} + \sum \vec{F} = \underbrace{\rho Q \vec{v}_2}_{\vec{I}\text{ im Ausgang}}$$

		\item \textbf{$x$-Achse:}
		$$\rho Q v_{1} + p_1 A_{1} - p_2 A_{2} + F_{tubo \rightarrow fl, x} = 0$$
		
		\item \textbf{$y$-Achse:} ($m$: ges. Fluidmasse im Rohr)
...
\begin{center}
	\textit{Die Druckkräfte wirken immer nach innen auf das Kontrollvolumen.} ($p \cdot A$)
\end{center}